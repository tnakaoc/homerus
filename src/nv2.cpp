...
using bd_ch_t=ch_t< std::vector<size_t> >;
	...
	++max_id;
	vector<trg_t> trg;
	for(size_t i=0;i<max_id;++i) trg.push_back(trg_t{i});
	bool start{false};
	auto print_info=[&](FILE* fp)
	{
		...
		fprintf(fp,"# ebin        = %lu,%lf,%lf\n",ebin.wid,e_min,e_max);
		fprintf(fp,"# tbin        = %lu,%lu,%lf,%lf\n",tbin.ste,tbin.mwid,tbin.min,tbin.max);
		fprintf(fp,"#\n");
		if(start)
		{
			fprintf(fp,"# trg :\n");
			fprintf(fp,"# id  : gated      | ungated\n");
			for(auto data:trg)
				fprintf(fp,"# %-4zu: %-10zu | %-10zu\n",data.hid,data.gated,data.ungated);
		}
	};
	print_info(stdout);
	bool update{false};
	...
	auto fill=[&](const vector<size_t>& l,double e,double t){
		if(l.empty()) return;
		for(auto id:l) ++trg[id].ungated;
		if(!((e>eg[0]&&e<eg[1])&&(t>tg[0]&&t<tg[1]))) return;
		for(auto id:l) ++trg[id].gated;
	...
	for(auto data:hom)
	{
		...
		fill(
			map[b][c],
			ecal.size()<b?e:ecal[b][c].a*(dis(eng)+e)+ecal[b][c].b,
			tcal.size()<b?t:tcal[b][c].a*(dis(eng)+t)+tcal[b][c].b
		);
		if(ctr=='q')
		{
			break;
